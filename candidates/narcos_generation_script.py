...
from sage.all import divisors, Integer, log, e, exp, euler_phi, hilbert_class_polynomial
...
if __name__ == '__main__':
    r = 0xffffffff00000001000000000000000000000000ffffffffffffffffffffffff
    k = 6
    potential_curves = []
    ...
    for i in range(n_iterations):
        ...
        poly = hilbert_class_polynomial(D)
        j_inv = poly.any_root(GF(q))
        if j_inv == 0 or j_inv == 1728 or D >= -388:
            if q.nbits() <= 512:
                ...
                if bits_of_sec_conservative >= 100:
                    bits_of_sec_nfs = test_finite_field_nfs(q, r, k, bits_of_sec_conservative, samples=100000, conj=True, max_coeff=3, B0_alpha=800, B1_alpha=2000, compute_alpha=True)
                    bits_of_sec_tnfs = test_finite_field_tnfs(q, r, k, bits_of_sec_conservative,  conj=True, max_coeff=2, B0_alpha=800, B1_alpha=1200, compute_alpha=True, alpha_test_principal=False)
